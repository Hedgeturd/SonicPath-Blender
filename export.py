...
def processBlenderObjects(context):
...
class ExportSonicPath(Operator, ExportHelper):
    bl_idname = "export_mesh.path_file"
    ...
    def execute(self, context):
        ...
        return self.processData(context)

    def processData(self, context):
        try:
            # Get parameters from Blender curves
            blenderVars = processBlenderObjects(context)
            ...
            writeXML(self.filepath, root)
            self.report({'INFO'}, f"Successfully imported SonicPath")
            return {'FINISHED'}
